...
npm run test:ficha-cliente                 # tests de la ficha de cliente (centros derivados, trabajos abiertos, historial)
...
- `src/app/gestor/` — panel interno, con login Supabase Auth:
  - `gestor/luz/` — Gestión Luz: cartera energética. El menú está organizado en **5 bloques por forma de trabajar** (`layout.tsx`), no por tipo de dato:
    ...
    - **Estudios y propuestas** (`gestor/luz/estudios`, tabla `luz_estudios`, lógica en `src/lib/factura.ts` + `src/lib/escenarios.ts`) — el paso que faltaba entre tener la factura y tener una oferta. El CRM decía «falta el estudio» y no había ningún sitio donde el estudio viviera: se leía la factura, se calculaba un ahorro, se hacía un PDF y todo eso se perdía. Lo caro no era repetir el trabajo, era **no poder contestar** cuando el cliente llama dos meses después preguntando por «los 1.400 € que me dijisteis».
      ...
      - **Un número del .xlsx llega como NÚMERO, nunca como texto** (`src/lib/excel-hojas.ts`). En el archivo, un consumo de 473,21 kWh está guardado como `473.21`; leyendo la celda **formateada** salía `«473.210»` —el formato de la plantilla pinta tres decimales— y al reinterpretarla ese punto parecía de miles: **473.210 kWh, mil veces más**, dentro de una oferta y sin ningún error. El archivo ya sabe el número; convertirlo a texto para volver a adivinarlo es inventarse un problema que no existía.
      - **Se abre con DOS motores.** ExcelJS falla con «Cannot read properties of undefined (reading "sheets")» en archivos válidos cuando el zip viene en otro orden — que es justo lo que hace Excel al guardar una plantilla generada por nosotros: la descargas, la rellenas, la guardas y ya no se puede abrir. Se prueba ExcelJS y se cae a **SheetJS**, que los lee sin pestañear. `excel-lectura.ts` ya lo había aprendido para las importaciones.
      - **Lo que no cabe en la potencia contratada es imposible, no improbable.** El tope es `kW × 24 h × días`; por encima, el dato no puede ser. Es lo único que caza de verdad un punto de miles: 473.210 kWh en un mes parecen normales en una hoja de cálculo, pero con 6,9 kW el máximo son 4.988 kWh y no sale ni de lejos. Avisa, no bloquea.
      - **El maxímetro por encima de lo contratado** marca `periodosEnExceso` y el consejo se invierte: ahí toca **subir** potencia, no bajarla, porque los excesos ya se están facturando.
      ...
      - **El informe en PDF** (`src/lib/estudio-pdf.ts`, datos en `src/lib/estudio-completo.ts`) — A4 **vertical**, dibujado con jsPDF y no imprimiendo la pantalla, por lo mismo que `parte-pdf.ts`: texto vectorial y cada bloque comprueba si cabe antes de pintarse. Lleva resumen, lo que paga hoy desglosado por periodo, **mes a mes con días y maxímetro**, potencia contratada contra registrada, reactiva, excedentes, las alternativas con su riesgo, y las hipótesis.
      - **La comparativa es la mitad del trabajo**, y encima la que hace cualquier comparador de internet. La otra mitad —si sobra o falta potencia, si paga reactiva, qué vierte a la red, cómo se reparte el consumo— es la que justifica que exista un asesor, y sale solo de la plantilla. `estudio-completo.ts` no calcula nada por su cuenta: delega en `optimizarPotencias`, `diagnosticarReactiva` y `calcularCoste`.
      - **El informe NO exige la plantilla.** Sale igual con los datos metidos a mano: los bloques sin datos no se pintan y lo que falta se dice en la trazabilidad. Negar el PDF entero por no tener los doce meses era quedarse sin nada por no tenerlo todo. Los **maxímetros, la reactiva y los excedentes se pueden teclear** en el paso de datos, y con un maxímetro anual el análisis de potencia se hace igual — con una lectura por periodo en vez de doce, así que el optimizador le da menos confianza, que es lo que corresponde.
      - **«Crear el suministro con estos datos»** aparece cuando el cliente no tiene CUPS. Un estudio sin CUPS es un cálculo suelto: no se puede tramitar el cambio, no cuenta el preaviso y no entra en la Agenda. Los datos ya están en pantalla, así que obligar a ir a otra pantalla a teclearlos otra vez es asegurarse de que el suministro no se cree nunca. `aFechaISO` entiende `30/04/2027` y `2027-04-30`, y **descarta lo que no entiende en vez de inventarse una fecha** — una fecha de fin equivocada mueve el preaviso, y eso cuesta un año.
      - **UN BLOQUE SIN DATOS NO SE PINTA**, pero se dice en las hipótesis. Un apartado «Energía reactiva» lleno de guiones alarga el informe y no informa; un hueco callado parece que no aplica. Dicho en la trazabilidad, es una cosa que pedir en la próxima visita — y suele valer dinero.
    ...
    - **Ficha de cliente** (`gestor/luz/clientes/[id]`, lógica en `src/lib/ficha-cliente.ts` + `src/lib/ficha-suministro.ts`, pantalla en `pestanas.tsx`) — era una columna larguísima: cabecera, siguiente acción, cuatro indicadores, suministros y un plegable con oportunidades, fechas, contratos, comisiones, visitas, seguimiento e historial. Todo cierto y todo en el mismo sitio, así que **para cualquier cosa había que recorrerla entera**. Ahora son **cuatro pestañas y cada una contesta UNA pregunta**: Resumen (¿qué hay que hacer ahora?) · Centros y suministros (¿qué puntos tiene y cómo están?) · Documentos (¿qué papeles hay sin revisar?) · Historial (¿qué se habló y qué se acordó). La pestaña se recuerda en localStorage.
      - **«CENTRO» ES UN AGRUPAMIENTO, NO UNA TABLA NUEVA.** Un cliente con dos granjas ve dos bloques, y eso no exige una entidad `Centro` con su formulario y su migración: el suministro ya lleva su dirección y `agruparEnCentros` da el mismo resultado sin un dato más que rellenar. Pedir que alguien cree el centro **antes** de poder apuntar un CUPS es asegurarse de que el CUPS no se apunte — el que se ve en la puerta de una granja se teclea con tres datos o no se teclea. Cuando haga falta un centro con datos propios ya existe `energia_ubicaciones`.
      - Se agrupa por **código postal + calle normalizada** (`claveDeDireccion`), no por la cadena literal: «Ctra. de Toledo km 12» y «CTRA TOLEDO KM 12, 45520» son el mismo sitio escrito por dos personas, y si cada variante fuera un centro **un cliente con tres naves saldría con siete**. El CP además hace de **corte**: lo que va detrás es el municipio y la provincia, que se escriben de mil maneras y sobran para identificar el sitio. Límite conocido y asumido: con el CP al final («Villaluenga (Toledo), 45520») esa dirección hace centro aparte — **es un fallo visible** (salen dos bloques parecidos y se corrige la dirección), y fundir dos sitios distintos por listeza sería silencioso, que es lo que no puede pasar.
      - **Trabajos abiertos** (`trabajosAbiertos`) es la tabla del Resumen: qué hay en marcha, en qué centro, en qué situación y **quién lo mueve y cuándo**. **Lo que no tiene siguiente acción va primero de todo**, por delante de cualquier fase que parezca más avanzada: es la regla madre y es por donde se cae un cliente. **Lo aparcado va al final aunque no tenga tarea** — se vio mirando la pantalla, con una «Revisión eléctrica» aparcada saliendo la segunda y en rojo, desmintiendo su propio texto («parado a propósito»).
      ...
      - **NO hay un catálogo de fases nuevo.** La situación sale de `etapas.ts` y el bloqueo de `ficha-suministro.ts`. Si esta pantalla inventara sus fases, diría del mismo cliente una cosa y el Pipeline otra.
      - **El bloqueo dice QUÉ falta, nunca «pendiente»**: «falta el consumo anual, sin él no se puede calcular el ahorro» se resuelve; «pendiente» hay que investigarlo. Y **se dice junto a la fila que lo tiene mal**, no en un aviso al pie: un aviso general hay que traducirlo a qué suministro se refiere, y eso no se hace.
      - **Rojo solo para bloqueo, vencimiento o riesgo real.** Una tarea futura no es roja. Y cada nivel lleva **texto obligatorio** además del color: con sol en la pantalla del móvil, el borde de color no se ve.
      - **EN MÓVIL NO HAY TABLAS, HAY BLOQUES APILADOS.** Medido en la vista previa a 390 px: «Situación» y «Próximo paso» —las dos columnas por las que se abre esto— se quedaban fuera de la pantalla, y un scroll horizontal dentro de una tarjeta no se ve ni se busca, así que la fila se leía a medias **y en silencio**.
      - **El historial junta gestiones, documentos y acuerdos** en una línea de tiempo por días, con «Hoy» y «Ayer» escritos como los diría una persona. **Los cambios del sistema vienen apagados y marcados**: un «se modificó el campo observaciones» junto a «el cliente dijo que se lo piensa» hace que el segundo no se lea. No se borran, porque el día que hay que reconstruir qué pasó son lo único que queda.
      - **El preaviso se vigila aunque el suministro esté ACTIVO**, y **cuando no se puede calcular se dice**. Ver la ficha del suministro.
      - **No se ha borrado nada**: los datos del cliente, las oportunidades, las fechas, los contratos, las comisiones, las visitas y el seguimiento siguen enteros en el plegable «Ficha completa» del Resumen. Lo que sí desapareció es `resumen.tsx`, cuya presentación quedó sustituida entera por `pestanas.tsx`.
      - Cubierto por `npm run test:ficha` y `npm run test:ficha-cliente`.
    - **Ficha del suministro** (`gestor/luz/cups/[id]`, lógica en `src/lib/formulario-suministro.ts`, formulario en `cups/formulario.tsx`) — un CUPS no tenía ficha: se editaba en una fila de una tabla de doce columnas o en una rejilla dentro de la ficha del cliente. Servía para corregir un dato suelto y para nada más; **no había ningún sitio donde mirar QUÉ LE PASA a un suministro**. Se llega desde el CUPS del listado y desde «ficha ↗» en la ficha del cliente (pulsar la fila sigue abriendo la edición rápida de siempre).
      ...
      - `TIPOS_CONTRATO` estaba **solo en un comentario de `supabase_luz.sql`**, así que cada formulario se lo inventaba. Ahora vive en `luz.ts` con los demás vocabularios.
      - Cubierto por `npm run test:suministro`.
    ...
    - **Bandeja** (`gestor/luz/bandeja`, lógica en `src/lib/bandeja.ts`) — la pantalla de Nicola: qué está esperando a que alguien lo meta o lo mueva. **No ordena por fecha sino por a quién bloquea**: bloquea la venta (David no puede ofertar) → bloquea el cobro → esperando al cliente → sus tareas. Dentro de cada grupo sí manda el tiempo parado.
      ...
      - **Ocho tandas con nombre** (`BANDEJAS` + `aplicarBandeja`): Todo · Mis vencidas · Datos de factura por meter · Ofertas por preparar · Esperando respuesta · Contratos bloqueados · Activaciones · Cobros pendientes. La bandeja ya sabía filtrar por grupo, tipo y texto, pero **montar la combinación costaba tres clics cada vez**, y al tercer día se deja de montar y se trabaja con la lista entera — justo lo que la bandeja venía a evitar. Son **fijas y no configurables a propósito**: no son el gusto de cada uno, son las tandas que existen, y con nombre se pueden pedir en voz alta. Las vacías no se pintan.
      - Para eso cada línea lleva un **`asunto`**, que no es el `grupo` (a quién bloquea) ni el `tipo` (qué se hace con las manos). Es un **campo y no un prefijo de la clave**: la clave es la identidad de la línea, y sacarle la categoría partiéndola por guiones se rompe callando el día que alguien renombra una.
      - Los contadores de montones y tipos se calculan **sobre la tanda elegida**: una pastilla que dijera «tramitar 12» y al pulsarla enseñara 3 deja de significar nada.
    ...
    - **Automatizaciones de fase** (`gestor/luz/automatismos`, solo admin; reglas en `src/lib/automatismos.ts`) — el trabajo que **debería existir por la fase de cada expediente y todavía no existe**: factura pedida sin nadie detrás, factura en casa y estudio sin empezar, oferta enviada sin seguimiento, contrato sin firmar, firmado sin enviar, en tramitación sin vigilancia, activado sin revisar la primera factura, preaviso acercándose y comisión con el cobro vencido.
      ...
      - Cubierto por `npm run test:automatismos`.
    - **Listado de clientes: vistas guardadas y columnas** (`gestor/luz/clientes`, lógica en `src/lib/vistas-listado.ts`, almacenamiento en `/api/luz/vistas`) — un listado con catorce columnas y once filtros es una herramienta potentísima que **nadie usa dos veces**, porque montarla cuesta medio minuto cada mañana. **La vista guardada es lo que convierte esa configuración en un botón**; sin vistas, las columnas configurables solo añaden trabajo. Por eso van juntas.
      - Vienen **seis puestas** —Todos, Parados de más, Clientes de verdad, Objetivos por zona, Les faltan datos, Sin responsable—, que no son ejemplos sino las preguntas que ya se montaban a mano. Las propias se pueden **compartir o no**: si todas fueran de equipo, la barra se llenaría de vistas de otro y la propia dejaría de encontrarse. Guardar con un nombre que ya existe **pisa** la vista en vez de añadir otra.
      - **Viven en `luz_config`, no en el navegador**, por lo mismo que los favoritos del seguimiento: la vista se monta en el ordenador y se mira en el móvil, y las compartidas no existirían si cada navegador tuviera las suyas. Lo que sí se queda en local es la última configuración de pantalla, que es «cómo lo dejé», no «cómo trabajo». **Todo lo guardado se valida al leerlo**: una vista con una columna que ya no existe degrada a la de fábrica, nunca deja la tabla en blanco.
      - **Elegir columnas cambia cuáles se ven, nunca en qué orden.** Dejar reordenar añade una decisión más a cambio de nada y hace que dos personas mirando la misma pantalla dejen de ver lo mismo.
      - Filtro de **estado de seguimiento** (parados de más · dentro de plazo · sin ninguna señal), que contesta en el listado «¿a quién estoy dejando parado?». **No tiene criterio propio**: los plazos salen de `seguimiento.ts` y la etapa de `etapas.ts`. «Sin ninguna señal» es **su propio cajón** y no se mete con los parados: a uno hay que llamarle, el otro entró de una importación y hay que mirarlo.
      - Cubierto por `npm run test:vistas`.
