...
// ── Leer un consumo escrito a mano ──────────────────────────────────────────
// En España el punto es separador de miles, pero parseFloat lo lee como decimal:
// quien teclea «53.558» acaba guardando 53,558 kWh, mil veces menos. Con eso la
// calculadora FV y cualquier comparativa salen mal, así que aquí se decide con
// reglas explícitas y se avisa cuando el valor no se puede deducir sin la factura.
export type ConsumoLeido = { kwh: number; sospechoso: boolean; motivo: string | null };

export function leerConsumo(bruto: string | number | null | undefined): ConsumoLeido {
  if (typeof bruto === 'number') return revisarConsumo(bruto, null);
  const s = String(bruto ?? '').trim().replace(/\s|kwh|kw\/h|kw·h/gi, '');
  if (!s) return { kwh: 0, sospechoso: false, motivo: null };

  const negativo = s.startsWith('-');
  const cuerpo = s.replace(/^[+-]/, '').replace(/[^\d.,]/g, '');
  if (!cuerpo) return { kwh: 0, sospechoso: false, motivo: null };
  ...
  if (!Number.isFinite(n)) return { kwh: 0, sospechoso: true, motivo: motivo ?? 'No es un número.' };
...
function revisarConsumo(kwh: number, motivo: string | null): ConsumoLeido {
  const redondeado = Math.round(kwh);
  if (motivo) return { kwh: redondeado, sospechoso: true, motivo };
  if (redondeado < 0) return { kwh: 0, sospechoso: true, motivo: 'El consumo no puede ser negativo.' };
