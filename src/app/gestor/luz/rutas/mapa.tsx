...
export interface ParadaMapa {
  ...
  interesFV?: boolean;
  /** Zona elegida a mano en la ficha del cliente (manda sobre la detección). */
...
interface Props {
  ...
  /** Coordenadas geocodificadas: la página las usa para asignar zona a todos. */
...
export function MapaRutas({ paradas, seleccion, onAlternar, orden, origenGeo, origenTexto, onRecargarClientes, modoManual, onMarcarFV, onUbicaciones, prospectos, onProspectoARuta, prospectosAnadidos, onResolverVisita }: Props) {
  ...
  useEffect(() => {
    ...
    for (const p of paradas) {
      ...
      // No se pinta en el pin (metía ruido), pero sí se muestra en el globo.
      ...
      div.innerHTML = `
        ...
        ${zona ? `<p style="font-size:11px;font-weight:700;margin-bottom:4px;color:${zona.color}">🗂️ ${zona.nombre}${p.zonaManual ? '' : ' (automática)'}</p>` : ''}
